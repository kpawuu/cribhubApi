 ...
 *  - identityChange
 */
export const authNotifier = (app: Application) => {
  return async (type: string, user: Record<string, any>, notifierOptions: Record<string, any> = {}): Promise<void> => {
    ...
    switch (type) {
      case 'resendVerifySignup': {
        ...
        await sendEmail(toEmail, `${appName} verification code`, await emailMessage(msg))
        break
      }
      case 'verifySignup': {
        const msg = `Hello ${user.fullName || ''}, your email has been verified successfully.`
        await sendEmail(toEmail, `${appName} email verified`, await emailMessage(msg))
        break
      }
      case 'sendResetPwd': {
        ...
        await sendEmail(toEmail, `${appName} password reset`, await emailMessage(msg))
        break
      }
      case 'resetPwd':
      case 'resetPwdShort': {
        const msg = `Hello ${user.fullName || ''}, your password has been reset successfully.`
        await sendEmail(toEmail, `${appName} password reset successful`, await emailMessage(msg))
        break
      }
      case 'passwordChange': {
        const msg = `Hello ${user.fullName || ''}, your password has been changed successfully.`
        await sendEmail(toEmail, `${appName} password changed`, await emailMessage(msg))
        break
      }
      case 'identityChange': {
        const msg = `Hello ${user.fullName || ''}, your account details were updated successfully.`
        await sendEmail(toEmail, `${appName} account updated`, await emailMessage(msg))
        break
      }
      default: {
        // eslint-disable-next-line no-console
