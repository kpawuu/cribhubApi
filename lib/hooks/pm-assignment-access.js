...
exports.restrictUnitsToAssignedPropertiesForPm = exports.requirePmAssignedToProperty = exports.restrictAgentPropertiesFind = exports.restrictPropertyManagerPropertiesFind = exports.getPmAssignedPropertyIds = void 0;
 ...
 */
const restrictAgentPropertiesFind = async (context) => {
    if (context.method !== 'find' || !context.params.provider)
        return context;
    const user = context.params.user;
    if (!user?._id)
        return context;
    const roles = Array.isArray(user.roles) ? user.roles : [];
    if (!roles.includes('agent') || roles.includes('admin') || roles.includes('landlord'))
        return context;
    const q = { ...(context.params.query || {}) };
    ...
    if (!portfolio)
        return context;
    const res = (await context.app.service('agent-assignments').find({ paginate: false, query: { agentUserId: user._id.toString() } }, { provider: undefined }));
    const list = Array.isArray(res) ? res : res?.data || [];
    const ids = list.map((a) => String(a.propertyId)).filter(Boolean);
    const q2 = { ...(context.params.query || {}) };
    if (!ids.length) {
        q2._id = { $in: [] };
    }
...
exports.restrictAgentPropertiesFind = restrictAgentPropertiesFind;
