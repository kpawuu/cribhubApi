...
const welcomeAgentOnAssignment = async (context) => {
    const r = context.result;
    if (!r?.agentUserId || !r?.propertyId)
        return context;
    ...
    try {
        const prop = await context.app
            ...
            .catch(() => null);
        const landlordId = String(prop?.landlordId || '');
        await (0, create_user_notification_1.createUserNotification)(context.app, {
            ...
            body: prop?.name
                ? `You are now the agent for "${prop.name}". Open the listing to coordinate with the landlord.`
