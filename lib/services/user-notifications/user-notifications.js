...
const notification_templates_1 = require("../../utils/notification-templates");
...
const notificationSmsGloballyDisabled = () => String(process.env.NOTIFICATION_SMS_DISABLED || '').toLowerCase() === 'true';
...
const emailRecipientOnNotificationCreated = async (context) => {
    ...
    try {
        ...
        if (recipient?.emailNotifications === false)
            return context;
        const parts = (0, notification_templates_1.renderEmailParts)({
        ...
        }, String(recipient.fullName || 'there'));
        const html = await (0, emailMessage_1.default)(parts.innerHtml, parts.ctaUrl, parts.ctaLabel);
        const textLines = [
            `Hi ${String(recipient.fullName || 'there')},`,
            '',
            parts.heading,
 ...
 */
const smsRecipientOnNotificationCreated = async (context) => {
    const n = context.result;
    if (!n?.userId || !n?.title)
        return context;
    if (context.params.skipNotificationSms)
        return context;
    if (notificationSmsGloballyDisabled())
        return context;
    if (!smsConfigured())
        return context;
    try {
        const recipient = await context.app.service('users').get(n.userId, { provider: undefined });
        const r = recipient;
        if (r?.smsNotifications === false)
            return context;
        const phone = normalizePhone(r?.phone);
        if (!phone)
            return context;
        const text = (0, notification_templates_1.renderSmsBody)({
            userId: String(n.userId),
            eventKey: String(n.eventKey || ''),
            category: String(n.category || ''),
            title: String(n.title),
        ...
        if (!text)
            return context;
        await context.app.service('sms').create({ recipient: [phone], message: text }, { provider: undefined });
    }
    catch (e) {
        // eslint-disable-next-line no-console
