...
class DashboardService {
    ...
    async find(_params) {
        ...
        if (role === 'property_manager') {
            ...
            const [assignedProperties, units, inquiries, rentalApplications, maintenanceRequests, payments, rentalContracts, pendingRequests, payoutsDue, unreadMessages] = await Promise.all([
        ...
        if (role === 'agent') {
            ...
            const unitQuery = unitIdList.length === 0 ? null : unitIdList.length === 1 ? { unitId: unitIdList[0] } : { unitId: { $in: unitIdList } };
            const unitsFilter = ids.length === 0 ? null : ids.length === 1 ? { propertyId: ids[0] } : { propertyId: { $in: ids } };
            const [assignedProperties, units, inquiries, rentalApplications, maintenanceRequests, openInquiries, pendingRequests, payoutsDue] = await Promise.all([
                Promise.resolve(ids.length),
                unitsFilter ? countColl(app, 'units', unitsFilter) : Promise.resolve(0),
