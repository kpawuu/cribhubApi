...
async function notifyInquiryCreated(app, inquiry) {
    ...
    try {
        ...
        if (landlordEmail) {
            ...
            await sendEmail(landlordEmail, `${appName}: new inquiry`, await (0, emailMessage_1.default)(msg, dashboardUrl));
        }
        if (landlordPhone) {
        ...
        if (agentUserId && agentUserId !== landlordId) {
            ...
            if (agentEmail) {
                ...
                await sendEmail(agentEmail, `${appName}: new inquiry (assigned)`, await (0, emailMessage_1.default)(msg, dashboardUrl));
            }
            if (agentPhone) {
